 ...
 */
export function splitByDoubleQuotes(input: string) {
	/**
	...
	const doubleQuotesMatcher = /[^\s]+(?:\\)+"|(?:\\)"[^"\s]*|[^"\s]+|"(?:\\"|[^"])*(?:[^\\"]")/gm,
		 ...
		 */
		args = input.match(doubleQuotesMatcher) ?? [];

	return args.map(arg => arg.replace(/(?<!\\)"/gm, "").replaceAll("\\", ""));
}
 ...
 */
export function addDoubleQuotes(input: string) {
 ...
 */
export function removeDoubleQuotes(input: string) {
